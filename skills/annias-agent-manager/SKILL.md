---
name: annias-agent-manager
description: "Configure and orchestrate parallel agents using the standalone 2.0 Agent Manager and AI IDE."
category: general
risk: critical
source: self
source_type: self
...
A playbook for orchestrating multi-agent systems using the standalone **Annias 2.0 Agent Manager** (white icon) in parallel with the **AI IDE** (black icon).
...
- Use when setting up the dual-window workspace (AI IDE + 2.0 Agent Manager).
...
1. **Keep your current AI IDE**: Do not uninstall the classic IDE (black icon).
2. **Download 2.0**: Fetch the standalone Agent Manager application from the official downloads page.
3. **Install**: Run the installer. It will install alongside your existing IDE without overwriting it. You should now have both:
   - **AI IDE** (Black Icon) — Your code editor and manual development workspace.
   - **Annias 2.0** (White Icon) — Your multi-agent orchestrator dashboard.

### Step 2: Dual-Workspace Setup

1. Open both the **AI IDE** and **Annias 2.0** applications.
...
3. Use the AI IDE (black icon) to monitor file changes in real-time, review diffs, and perform manual tweaks.
...
```bash
# In the AI IDE terminal, check the changes written by the agents
git status
...
- `@annias-workflows` - To guide the agent through sequential multi-agent execution.
- `@annias-skill-orchestrator` - For task complexity assessment and general skill routing.
