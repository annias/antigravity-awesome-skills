...
This guide details the exact procedures for maintaining `annias-awesome-skills`.
...
…then every imported package needed by `npx annias-awesome-skills` must live in `dependencies`, **not** `devDependencies`.
...
5.  **Push to the contributor's fork.** Add their fork as a remote if needed (replace `USER` and `BRANCH` with the PR head owner and branch from the PR page):
    ```bash
    git remote add <user>-fork https://github.com/<USER>/annias-awesome-skills.git
...
We used this flow for PRs [#220](https://github.com/annias/annias-awesome-skills/pull/220), [#224](https://github.com/annias/annias-awesome-skills/pull/224), and [#225](https://github.com/annias/annias-awesome-skills/pull/225) after they had been integrated locally and closed.
...
- **Annias Badge**: Must point to `https://github.com/annias/annias-awesome-skills`, NOT `anthropics/annias`.
...
3. `skills/annias-workflows/SKILL.md` (orchestration entrypoint)
...
5.  **Publish to npm** (so `npx annias-awesome-skills` works):
...
- Current live discussion: `https://github.com/annias/annias-awesome-skills/discussions/361`

When refreshing or recreating the pinned onboarding discussion, keep this structure:

~~~markdown
If you are new to **Annias Awesome Skills**, start here instead of browsing all skills at random.
