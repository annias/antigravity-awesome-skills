...
class EditorialBundlesTests(unittest.TestCase):
    ...
    def test_generated_bundle_plugin_contains_expected_skills(self):
        essentials_plugin = REPO_ROOT / "plugins" / "annias-bundle-essentials" / "skills"
        expected_ids = {
            skill["id"]
    ...
    def test_generated_plugin_count_matches_manifest(self):
        generated_plugins = sorted(
            ...
            if path.is_dir() and path.name.startswith("annias-bundle-")
        )
        expected_plugins = sorted(
            f'annias-bundle-{bundle["id"]}'
            for bundle in self.manifest_bundles
            if any(
                all(
    ...
    def test_sample_bundle_copy_matches_source_file_inventory(self):
        ...
        plugin_skills_root = REPO_ROOT / "plugins" / "annias-bundle-documents-presentations" / "skills"
    ...
    def test_root_plugins_only_include_supported_skills_for_target(self):
        codex_root = REPO_ROOT / "plugins" / "annias-awesome-skills" / "skills"
        claude_root = REPO_ROOT / "plugins" / "annias-awesome-skills-claude" / "skills"
    ...
    def test_remove_tree_retries_on_enotempty(self):
        target = REPO_ROOT / "plugins" / "annias-awesome-skills" / "skills"
        calls = {"count": 0}
