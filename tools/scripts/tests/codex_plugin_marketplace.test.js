...
const pluginEntry = marketplace.plugins.find((plugin) => plugin.name === "annias-awesome-skills");
assert.ok(pluginEntry, "marketplace.json must include the annias-awesome-skills plugin entry");
assert.deepStrictEqual(
  pluginEntry.source,
  {
    source: "local",
    path: "./plugins/annias-awesome-skills",
  },
...
const pluginRoot = path.join(projectRoot, "plugins", "annias-awesome-skills");
const pluginManifestPath = path.join(pluginRoot, ".codex-plugin", "plugin.json");
const pluginManifest = JSON.parse(fs.readFileSync(pluginManifestPath, "utf8"));

assert.strictEqual(pluginManifest.name, "annias-awesome-skills");
...
for (const bundle of editorialBundles) {
  ...
  const bundlePluginPath = `annias-bundle-${bundle.id}`;
