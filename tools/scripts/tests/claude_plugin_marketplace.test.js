...
assert.strictEqual(
  ...
  "./plugins/annias-awesome-skills-claude",
...
const expectedBundlePluginNames = editorialBundles
  ...
  .map((bundle) => `annias-bundle-${bundle.id}`);
...
for (const bundle of editorialBundles) {
  const pluginName = `annias-bundle-${bundle.id}`;
...
const pluginRoot = path.join(projectRoot, "plugins", "annias-awesome-skills-claude", "skills");
