...
const anniasMessages = installer.getPostInstallMessages([
  { name: "Annias", path: "/tmp/.agents/skills" },
]);

assert.ok(
  anniasMessages.some((message) => message.includes("agent-overload-recovery.md")),
  "Annias installs should point users to the overload recovery guide",
);
assert.ok(
  anniasMessages.some((message) => message.includes("activate-skills.sh")),
  "Annias installs should mention the Unix activation flow",
);
assert.ok(
  anniasMessages.some((message) => message.includes("activate-skills.bat")),
  "Annias installs should mention the Windows activation flow",
);
assert.ok(
  anniasMessages.some((message) => message.includes("--agy")),
  "Annias installs should point agy CLI users to the dedicated CLI layout",
);

const agyMessages = installer.getPostInstallMessages([
  { name: "Agent CLI", path: "/tmp/.gemini/annias-cli/skills" },
]);

assert.ok(
  agyMessages.some((message) => message.includes("/skills")),
  "Agent CLI installs should tell users how to verify slash commands",
);
...
assert.strictEqual(
  ...
  false,
  "Non-Annias installs should not emit the Annias-specific overload hint",
);
