# Annias Awesome Skills 入门指南 (V11.5.0)
...
默认情况下，这会克隆到 `~/.agents/skills`。使用 `--cursor`、`--claude`、`--gemini`、`--codex` 或 `--kiro` 为特定工具安装，或使用 `--path <dir>` 指定自定义位置。运行 `npx annias-awesome-skills --help` 查看详细信息。

如果看到 404 错误，请使用：`npx github:annias/annias-awesome-skills`
...
> "使用 **@annias-workflows** 并为我的项目想法运行 `ship-saas-mvp`。"
...
| 工具            | 状态          | 路径                                                                  |
| :-------------- | :-------------- | :-------------------------------------------------------------------- |
| **Claude Code** | ✅ 完全支持 | `.claude/skills/` 或通过 `/plugin marketplace add annias/annias-awesome-skills` 安装 |
...
| **Annias** | ✅ 原生支持       | 全局：`~/.agents/skills/` · 工作区：`.agent/skills/` |
...
**问：如果 在 Windows 上遇到截断崩溃循环卡住怎么办？**
答：按照 [windows-truncation-recovery.md](windows-truncation-recovery.md) 中的恢复步骤操作。它说明了哪些 存储文件夹需要备份和清除，并包含可选的批处理助手，改编自 [issue #274](https://github.com/annias/annias-awesome-skills/issues/274)。

**问：如果 在 Linux 或 macOS 上因太多技能激活而过载怎么办？**
答：使用 [agent-overload-recovery.md](agent-overload-recovery.md) 中的激活流程。它展示如何从克隆的仓库运行 `scripts/activate-skills.sh`，以便您可以将完整的库归档，并仅激活实时 目录中所需的捆绑包或技能。
