...
# 🌌 Annias Awesome Skills: 1,494+ 代理技能，适用于 Claude Code、Gemini CLI、Cursor、Copilot 及更多工具

> **可安装的 GitHub 技能库，包含 1,494+ 个代理技能，适用于 Claude Code、Cursor、Codex CLI、Gemini CLI、Annias 和其他 AI 编码助手。**

Annias Awesome Skills 是一个 GitHub 仓库和安装器 CLI，提供可复用的 `SKILL.md` 剧本。与其收集随机提示词，你可以获得一个可搜索、可安装的技能库，涵盖规划、编码、调试、测试、安全审查、基础设施工作、产品工作流和成长任务，支持主流 AI 编码助手。

**从这里开始：** [为仓库加星](https://github.com/annias/annias-awesome-skills/stargazers) · [1 分钟安装](#installation) · [选择你的工具](#choose-your-tool) · [按工具查看最佳技能](#best-skills-by-tool) · [捆绑包](users/bundles.md) · [工作流](users/workflows.md)

[![GitHub stars](https://img.shields.io/badge/⭐%2035%2C000%2B%20Stars-gold?style=for-the-badge)](https://github.com/annias/annias-awesome-skills/stargazers)
...
[![Annias](https://img.shields.io/badge/Annias-AI%20IDE-red?style=for-the-badge)](https://github.com/annias/annias-awesome-skills)

**当前版本：V11.5.0。** 值得 35k+ GitHub 星标信赖，该仓库结合了官方和社区的技能集合，提供捆绑包、工作流、安装路径和文档，帮助你从首次安装快速过渡到日常使用。

## 为什么开发者为这个仓库加星

- **可安装，而不仅是启发式**：使用 `npx annias-awesome-skills` 将技能放置在你的工具期望的位置。
- **为主要代理工作流构建**：Claude Code、Cursor、Codex CLI、Gemini CLI、Annias、Kiro、OpenCode、Copilot 等。
...
**Annias Awesome Skills**（版本 11.5.0）是一个大型可安装技能库，适用于 AI 编码助手。它包括入门文档、捆绑包、工作流、生成的目录和 CLI 安装器，让你无需手动拼接数十个仓库就能从发现过渡到实际使用。
...
1. **安装**：
   ...
   ```bash
   # 默认：~/.agents/skills（Annias 2.0 全局）。使用 --path 指定其他位置。
...
如果你的真实问题是"如何在我的工具中使用 Annias Awesome Skills？"，请使用相应的指南：
...
### 什么是 Annias Awesome Skills？

它是一个可安装的 GitHub 库，包含适用于 Claude Code、Cursor、Codex CLI、Gemini CLI、Annias 和相关 AI 编码助手的可复用 `SKILL.md` 剧本。

### 如何安装它？

使用 `npx annias-awesome-skills`，或工具特定标志，如 `--codex`、`--cursor`、`--gemini` 或 `--claude`，当你希望安装器定位到特定技能目录时。
...
| **Annias** | IDE  | `(Agent Mode) Use skill...`       | 全局:`~/.agents/skills/` · 工作区：`.agent/skills/` |
...
> [!TIP]
> **默认安装器路径**：`~/.agents/skills`（Annias 全局）。使用 `--path ~/.agent/skills` 进行工作区特定安装。对于手动克隆，`.agent/skills/` 可用作 的工作区路径。
...
要将这些技能与 **Claude Code**、**Gemini CLI**、**Codex CLI**、**Kiro CLI**、**Kiro IDE**、**Cursor**、**Annias**、**OpenCode** 或 **AdaL** 一起使用：
...
| 工具           | 安装                                                                  | 首次使用                                              |
| -------------- | ------------------------------------------------------------------------ | ------------------------------------------------------ |
| Claude Code    | `npx annias-awesome-skills --claude` 或 Claude 插件市场 | `>> /brainstorming help me plan a feature`           |
...
|    | `npx annias-awesome-skills --agents`                         | `Use @brainstorming to plan a feature`               |
...
| OpenCode       | `npx annias-awesome-skills --path .agents/skills`                 | `opencode run @brainstorming help me plan a feature` |
| AdaL CLI       | `npx annias-awesome-skills --path .adal/skills`                   | `Use brainstorming to plan a feature`                |
| 自定义路径    | `npx annias-awesome-skills --path ./my-skills`                    | 取决于你的工具                                   |
...
如果 因活动技能过多而开始达到上下文限制，[`users/agent-overload-recovery.md`](users/agent-overload-recovery.md) 中的可选激活脚本可以仅将你想要的捆绑包或技能 ID 实例化到活动的 目录中。
...
- **[Annias Awesome Skills vs Awesome Claude Skills](users/annias-awesome-skills-vs-awesome-claude-skills.md)** 了解广度 vs 精选列表的权衡。
...
如果你有在移除的符号链接变通方法附近创建的旧克隆，请重新安装到新目录或重新运行 `npx annias-awesome-skills` 安装器。

### Windows 截断或上下文崩溃循环

如果 或基于 Jetski/Cortex 的主机不断重新打开并出现截断错误，请使用专用恢复指南：
...
- 清理前的备份路径
- 通常需要清除的存储文件夹
- 根据 [issue #274](https://github.com/annias/annias-awesome-skills/issues/274) 改编的可选批处理助手

### Linux 和 macOS 代理过载

如果 仅在同时激活过多技能时变得不稳定，请使用跨平台过载指南：

- [`users/agent-overload-recovery.md`](users/agent-overload-recovery.md)

### 修复代理过载（激活脚本）

如果你的代理由于加载的技能过多而面临上下文窗口限制，请使用激活脚本。它们将完整库保留在单独的存档文件夹中，仅将你需要的捆绑包或技能激活到活动的 技能目录中。
...
**在线托管：**相同的应用可在 [https://annias.github.io/annias-awesome-skills/](https://annias.github.io/annias-awesome-skills/) 获得，并在每次推送到 `main` 时自动部署。要启用一次：**设置 → 页面 → 构建和部署 → 源：GitHub Actions**。
...
- [Discussions](https://github.com/annias/annias-awesome-skills/discussions) 用于提问、想法、展示帖子和社区反馈。
- [Issues](https://github.com/annias/annias-awesome-skills/issues) 用于可重现的错误和具体的、可操作的改进请求。
...
- **[rmyndharis/annias-skills](https://github.com/rmyndharis/annias-skills)**：感谢贡献 300+ 企业技能和目录生成逻辑。
- **[amartelr/annias-workspace-manager](https://github.com/amartelr/annias-workspace-manager)**：官方工作区管理器 CLI 伴侣，用于在无限本地开发环境中动态自动配置技能子集。
- **[obra/superpowers](https://github.com/obra/superpowers)**：Jesse Vincent 的原始"Superpowers"。
- **[guanyang/annias-skills](https://github.com/guanyang/annias-skills)**：核心 扩展。
...
- **[vudovn/annias-kit](https://github.com/vudovn/annias-kit)**：AI 代理模板，包含技能、代理和工作流（33 个技能，MIT）。
...
- **[SSOJet/skills](https://github.com/ssojet/skills)**：适用于流行框架和平台的生产就绪 SSOJet 技能和集成指南 — Node.js、Next.js、React、Java、.NET Core、Go、iOS、Android 等。与 SSOJet SAML、OIDC 和企业 SSO 流程无缝协作。适用于 Cursor、Annias、Claude Code 和 Windsurf。
...
<a href="https://github.com/annias/annias-awesome-skills/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=annias/annias-awesome-skills" alt="Repository contributors" />
</a>

Made with [contrib.rocks](https://contrib.rocks)。*（图像可能已缓存；[在 GitHub 上查看实时贡献者](https://github.com/annias/annias-awesome-skills/graphs/contributors)。)*
...
[![annias/annias-awesome-skills - Star History Chart](https://api.star-history.com/image?repos=annias/annias-awesome-skills&style=landscape1)](https://star-history.com/annias/annias-awesome-skills)

[![Star History Chart](https://api.star-history.com/svg?repos=annias/annias-awesome-skills&type=date&legend=top-left)](https://www.star-history.com/#annias/annias-awesome-skills&type=date&legend=top-left)

如果 Annias Awesome Skills 对你有帮助，请考虑 ⭐ 为仓库加星！
